...
###ssh to server
#ssh -i /home/spkaabi/ec2/spkaabi.pem ubuntu@ec2-79-125-28-224.eu-west-1.compute.amazonaws.com

###sft to server
#sftp -o"IdentityFile=/home/spkaabi/ec2/spkaabi.pem" ubuntu@ec2-79-125-28-224.eu-west-1.compute.amazonaws.com
...
task :upload_data, :roles => group_name do
    run "rsync -e 'ssh -i /home/spkaabi/ec2/spkaabi.pem' -vzP data/* ubuntu@ec2-79-125-28-224.eu-west-1.compute.amazonaws.com:#{mount_point}" 
end
...
task :run_bowtie, :roles => group_name do
  ...
  files.each{|infile|
    ...
    run ("export BOWTIE_INDEXES='#{working_dir}/indexes' && bowtie  --sam --best -k1 -l15 -n1 -m3 -p20 --solexa-quals --chunkmbs 256  -q mm9 --quiet  #{infile}  > #{outfile} ")
  } 
...
task :run_macs, :roles => group_name do
  ...
  treatment = "#{mount_point}/chip_export_sorted_nodups.bam"
  control = "#{mount_point}/SC_input_export_sorted_nodups.bam"
  genome = 'mm'
  bws = [300]
  pvalues = [0.00001]
